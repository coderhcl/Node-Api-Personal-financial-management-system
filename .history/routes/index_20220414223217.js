module.exports = (app) => {
  ...
  // 获取收入 + 搜索，分页等功能
  router.post('/income/getincome', async (req, res) => {
  ...
  // 编辑公告
  router.patch('/income/patchincome/:id', async (req, res) => {
  ...
  // 图表数据查询
  router.post('/income/chart', async (req, res) => {
  ...
  // 获取收入一年数据
  router.post('/income/yearchart', async (req, res) => {
    // console.log(req.body)
    const result = await Income.find({
  ...
  // ############### 支出管理 #################
  // 添加支出
  router.post('/expense/addexpense', (req, res) => {
  ...
  // 获取支出 + 搜索，分页等功能
  router.post('/expense/getexpense', async (req, res) => {
    const { userId, offset, size, formData } = req.body
    // console.log(req.body.formData)
    const skipNumber = (offset - 1) * size
  ...
  // 编辑支出
  router.patch('/expense/patchexpense/:id', async (req, res) => {
  ...
  // 支出图表数据查询
  router.post('/expense/chart', async (req, res) => {
    const { userId, limitNum, createTime } = req.body
    // console.log(createTime)
    if (createTime.length !== 0) {
  ...
  // 获取支出一年数据
  router.post('/expense/yearchart', async (req, res) => {
    // console.log(req.body)
    const result = await Expense.find({
      userId: req.body.userId,
    })
  ...
  // ################ 债务 ######################
  // 添加债务
  router.post('/debt/adddebt', (req, res) => {
  ...
  // 获取支出 + 搜索，分页等功能
  router.post('/debt/getdebt', async (req, res) => {
    const { userId, offset, size, formData } = req.body
    const skipNumber = (offset - 1) * size
    if (formData) {
      ...
      const userList = await Debt.find({
        name: { $regex: formData.name },
        remark: { $regex: formData.remark },
      })
        .skip(skipNumber)
        .limit(size)
      if (formData.category && !formData.createTime && !formData.) {
        const totalCount = await Debt.find({
          name: { $regex: formData.name },
          phone: { $regex: formData.phone },
          email: { $regex: formData.email },
  ...
  // 删除支出信息
  router.delete('/debt/delete/:id', (req, res) => {
  ...
  // 编辑支出
  router.patch('/debt/patchdebt/:id', async (req, res) => {
  ...
  // 支出图表数据查询
  router.post('/debt/chart', async (req, res) => {
    const { userId, limitNum, createTime } = req.body
    // console.log(createTime)
    if (createTime.length !== 0) {
  ...
  // 获取支出一年数据
  router.post('/debt/yearchart', async (req, res) => {
    // console.log(req.body)
    const result = await Debt.find({
      userId: req.body.userId,
    })
