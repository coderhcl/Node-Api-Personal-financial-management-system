module.exports = (app) => {
  ...
  router.post('/income/getincome', async (req, res) => {
    ...
    // if (formData.category && !formData.remark && !formData.createTime) {
    //   const totalCount = await Income.find({
    //     userId: userId,
    ...
    //   })
    // }
    // if (formData.category && formData.remark) {
    //   const totalCount = await Income.find({
    //     userId: userId,
    //     category: formData.category,
    //     remark: { $regex: formData.remark },
    ...
    //   })
    // }
    const List = await Income.find({ userId: userId })
      .skip(skipNumber)
      .limit(size)
    const totalCount = await Income.find({ userId: userId })
    res.send({
      code: 1,
      data: {
        incomeList: List,
